...
#define SPIN_PRICE 5
#define SMALL_PRIZE 80
#define BIG_PRIZE 200
#define JACKPOT 2000
...
/obj/machinery/computer/slot_machine/ui_interact(mob/living/user)
	. = ..()
	var/reeltext = {"<center><font face=\"courier new\" size='4'>
	/*****^*****^*****^*****^*****\\<BR>
	...
	if(working)
		...
		<font size='1'>5 Credit Minimum, 100 Credit Maximum</center><BR>
		<font size='2'><B>Prize Money Available:</B> [money]<BR>
		<B>Credit Remaining:</B> [balance]<BR>
		<center>[plays] players have tried their luck today, and [jackpots] have won a jackpot!<BR>
		<HR>
		<font size='1'><A href='?src=[REF(src)];betminus=1'>Bet-</A><font size='3'><A href='?src=[REF(src)];spin=1'>Play!</A><font size='1'><A href='?src=[REF(src)];betplus=1'>Bet+</A><BR>
		<BR><font size='4'>
		[reeltext]
		<BR>
...
/obj/machinery/computer/slot_machine/proc/give_prizes(usrname, mob/user)
	...
	if(reels[1][2] + reels[2][2] + reels[3][2] + reels[4][2] + reels[5][2] == "[SEVEN][SEVEN][SEVEN][SEVEN][SEVEN]")
		...
		visible_message("<b>[src]</b> says, 'Big Winner! You win [(BIG_PRIZE * bet)] credits!'")
		balance += (BIG_PRIZE * bet)

	else if(linelength == 4)
		visible_message("<b>[src]</b> says, 'Winner! You win [(SMALL_PRIZE * bet)] credits!'")
		balance += (SMALL_PRIZE * bet)

	else if(linelength == 3)
		to_chat(user, "<span class='notice'>You win [(4 * bet)] games!</span>")
		balance += bet * 4
		money = max(money - bet * 4, money)
